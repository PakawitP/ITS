...
const TestModeExam: React.FC = () => {
    ...
    var dataTestMode: { [Resid: number]: any; } = {};
    // var dataResChioce: { [Pointid: number]: string; } = {};
    ...
    const { data: Responsive, error: errorResponsive } = useSWR(
    ...
    if (errorExam || errorquizSet || errorResponsive || errorTimeQuiz) {
        return (
            <div>
                {errorExam && (<div>
                    errorExam {errorExam}
                ...
                {errorTimeQuiz && (<div>
                    errorTimeQuiz {errorTimeQuiz}
                </div>)}
            </div>
        )
    };
    if (!Exam || !quizSet || !Responsive || !TimeQuiz) {
        return (
    ...
    const onSubmit = (finalAnswer: any) => {
        ...
        return axios.post(URL.API_SaveExam, {
            ...
            "quiz_set_id": examID,
            "quiz": finalAnswer.quizAnstemp,
            "totel_quiz": Exam.totel_id_quiz,
            "class": data_user.user_class,
            "data_test": finalAnswer.dataTest
        }, {
            headers: {
    ...
    const TestModeSheet = () => {
        ...
        for (let i = 0; i < Exam.data.length; i++) {
            ...
            dataTestMode[Exam.data[i].quiz_id] = []
            for (let j = 0; j < Responsive.data.length; j++) {

                dataTestMode[Exam.data[i].quiz_id].push("choiceOne")
            }
        }
                ...
                <div>
                    ...
                    {flip ?
                        ...
                        <ShowExam
                            ...
                            onSubmit={onSubmit}
                            Admin={false}
                            dataResponsive={Responsive.data}
                            timer={Exam.dis_quiz_set.quiz_set_time}
                            dataTestMode={TestModeSheet()}
